...
import { Calendar as CalendarIcon, Clock, ChevronLeft, ChevronRight, Filter, Info, Plane, Sun, Moon, Eye, EyeOff, ShoppingBag } from "lucide-react";
...
export default function CalendarView() {
  ...
  const getDutyPeriodForDate = (seq: SequenceTrip, date: Date): DutyPeriod | undefined => {
    ...
    // Parse sequence startDate into local components
    ...
    const diffTime = Math.abs(dateObj.getTime() - seqStart.getTime());
    const diffDays = Math.round(diffTime / (1000 * 60 * 60 * 24));
    
    return seq.dutyPeriods.find((dp) => dp.dayIndex === diffDays);
  };

  // Helper: check if a sequence is high credit (> 15 hours / 900 minutes)
  const isHighCredit = (seq: SequenceTrip) => {
    return seq.totalCreditMinutes >= 900;
  };
  ...
  const sequenceSegments = useMemo(() => {
    ...
    const segments: {
      seq: SequenceTrip & { isGhost?: boolean; hasConflict?: boolean };
      row: number;
      startCol: number;
      endCol: number;
      isRealStart?: boolean;
    ...
    const allTripsToRender = [...sequences];
    ...
    if (showOpenTimeOverlay) {
      openSequences.forEach((ot) => {
        ...
        (otTrip as any).isGhost = true;
        (otTrip as any).hasConflict = conflict.hasConflict;
    ...
    Object.keys(rowSegments).forEach((rowStr) => {
      ...
      // Sort: longer span first, then earlier startCol
      rowSegs.sort((a, b) => {
  ...
  const getSegmentBackground = (seg: any) => {
  ...
  return (
    <div className="space-y-6 relative animate-fadeIn">
      ...
      <div className="flex items-center justify-between bg-slate-900/40 backdrop-blur-md border border-slate-800/60 p-4 rounded-2xl shadow-xl">
        <button
          onClick={handlePrevMonth}
          className="p-2 hover:bg-slate-800 text-slate-400 hover:text-white rounded-xl border border-slate-800/80 hover:border-slate-700/80 transition duration-150"
        >
      ...
      {viewMode === "month" ? (
        ...
        <div className="bg-slate-950 border border-slate-900 rounded-3xl overflow-hidden shadow-2xl">
          ...
          <div className="grid grid-cols-7 bg-slate-900/10" style={{ gridTemplateRows: `repeat(6, minmax(${isMobile ? "75px" : "130px"}, auto))` }}>
              ...
              return (
                <div
                  ...
                  className={`relative p-3 border-r border-b border-slate-900/60 flex flex-col justify-between transition-all duration-200 ${
                    isCurrentMonth ? "bg-slate-950/20" : "bg-slate-950/5 opacity-30 pointer-events-none"
                  } ${hide ? "opacity-10" : ""}`}
                >
                  {/* Date badge */}
                  <div className="flex items-center justify-between">
                    ...
                    {/* Overnight layover label on corresponding day */}
                    {seq && dp && dp.layoverCity && dp.layoverCity !== seq.base && (
                      <span className="text-[8px] sm:text-[9px] font-black text-amber-400 bg-amber-950/60 border border-amber-500/20 px-1 sm:px-1.5 py-0.2 sm:py-0.5 rounded-md sm:rounded-lg flex items-center gap-0.5 sm:gap-1 shadow-sm z-20 whitespace-nowrap">
                        <Moon className="w-2.5 h-2.5 fill-amber-400/20 shrink-0" />
                        <span>{isMobile ? dp.layoverCity : `RON: ${dp.layoverCity}`}</span>
                      </span>
                    )}

                    {/* DFP label */}
                    {isDfp && filterMode !== "trips" && (
                      <span className="text-[8px] sm:text-[9px] font-bold text-slate-600 tracking-wide uppercase px-1 sm:px-1.5 py-0.2 sm:py-0.5 rounded bg-slate-900/80 border border-slate-800/40">
                        DFP
                      </span>
                    )}
                  </div>
            ...
            {sequenceSegments
              ...
              .map((seg, idx) => {
                const colorSet = getColorClasses(seg.seq.colorTag, seg.seq.id === selectedSequenceId);
                const isOt = seg.seq.isOvertime;
                const isSim = (seg.seq as any).isSimulated;
                const isGhost = (seg.seq as any).isGhost;
                const hasConflict = (seg.seq as any).hasConflict;
                ...
                const isRealStart = (seg as any).isRealStart;
                const isRealEnd = (seg as any).isRealEnd;
                ...
                let rightPct = (1 - (N - 1 + eFrac) / N) * 100;
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      alignSelf: "start",
                      marginTop: `${isMobile ? 22 + (seg as any).slot * 25 : 42 + (seg as any).slot * 46}px`,
                  ...
                  >
                    <div className="flex items-center justify-between text-[8px] sm:text-[10px] font-black tracking-wide leading-none">
                      <span className="flex items-center gap-1 sm:gap-1.5 truncate">
                        ...
                        <span className="truncate">{isMobile ? seg.seq.sequenceNumber : `Seq ${seg.seq.sequenceNumber}`}</span>
                        {!isMultiDay && (
                          <span className={`text-[8px] sm:text-[9px] font-semibold ml-0.5 sm:ml-1 shrink-0 ${subtextColor}`}>
                            ({(seg.seq.totalCreditMinutes / 60).toFixed(isMobile ? 0 : 1)}h)
                          </span>
                        )}
                      </span>
                    </div>
                    ...
                    {isMultiDay && !isMobile && (
                      <div className={`text-[9px] font-semibold leading-none truncate flex justify-between items-center font-sans ${subtextColor}`}>
                        <span className="flex items-center gap-0.5">
                          ...
                          <span>{(seg.seq.totalCreditMinutes / 60).toFixed(1)}h cr</span>
                        </span>
                        {seg.seq.layoverCities.length > 0 && (
        ...
        <div className="space-y-4">
          {weekDays.map((date, idx) => {
            ...
            const colorSet = seq ? getColorClasses(seq.colorTag, isSelected) : null;
            ...
            return (
              <div
                ...
                className={`p-4 bg-slate-900/30 backdrop-blur-md border rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition duration-200 ${
                  isToday ? "border-indigo-500/50 bg-indigo-950/5" : "border-slate-800/80"
              ...
              {(() => {
                ...
                if (!seq) {
                  ...
                  if (openSeq) {
                    ...
                    (seq as any).isGhost = true;
                    (seq as any).hasConflict = conflict.hasConflict;
                    (seq as any).conflictReason = conflict.reason;
                  }
                }
                if (!seq) return null;

                const isGhost = (seq as any).isGhost;
                const hasConflict = (seq as any).hasConflict;
                  ...
                  <div className="space-y-3">
                    ...
                    {hasConflict && (
                      <div className="p-2.5 bg-rose-950/30 border border-rose-900/40 rounded-xl text-[10px] text-rose-400 font-sans leading-relaxed">
                        <strong>Conflict:</strong> {(seq as any).conflictReason}
                      </div>
                    )}
